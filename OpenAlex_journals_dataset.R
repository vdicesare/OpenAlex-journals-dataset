...
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig3A.png", width = 6, height = 3, dpi = 300)


## FIGURE 3B
figure3B <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                       ...
                                       distinct(country, OA_source_ID, mains) %>%
                                       count(country, mains, name = "n_journals") %>%
                                       group_by(country) %>%
                                       mutate(total_journals = sum(n_journals),
                                              percent_journals = 100 * n_journals / total_journals,
...
figure3B <- world %>% left_join(figure3B %>% filter(mains == 1), by = c("admin" = "country"))

# group countries at the region level
figure3B <- figure3B %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
...
figure3B <- figure3B %>% select(name_long, region_custom, percent_journals, mains_label) %>%
...
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig3B.png", width = 9, height = 6, dpi = 600)
...
ggplot(figure6A, aes(x = perc, y = OA_main_domains, fill = mains_label)) +
  ...
  theme(strip.text = element_text(size = 12, face = "bold"), axis.text.y = element_text(size = 7),
        legend.position = "bottom", legend.direction = "horizontal", legend.key.size = unit(0.4, "cm"), legend.text = element_text(size = 7))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig6A.png", width = 6, height = 2, dpi = 300)


## FIGURE 6B
figure6B <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
...
# group countries at the region level
figure6B <- figure6B %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                          region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                          region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                          region_un == "Americas" & subregion == "South America" ~ "South America",
                                                          TRUE ~ NA_character_))
...
# convert to long format
figure6B <- figure6B %>% select(name_long, region_custom, percent_journals, mains_label) %>%
                         rename(percent = percent_journals)
...
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig6B.png", width = 9, height = 4, dpi = 600)


## FIGURE 6C
figure6C <- figure6B %>% filter(!is.na(percent), !is.na(region_custom)) %>%
                         group_by(region_custom) %>%
                         summarise(median_percent = median(percent, na.rm = TRUE)) %>%
                         mutate(condition = "All references")

# OpenAlex march 2025 version upload for computations that apply a threshold to references
openalex_threshold_references <- read.csv("~/Desktop/Local.Journals/local_journals_OA2503_journals_functions.csv")
openalex_threshold_references <- ddff_megamerge %>% filter(OA_source_ID %in% openalex_threshold_references$journal_id)
openalex_threshold_references <- openalex_threshold_references %>% filter(local_producers == "local")

# produce the same computations as in FIGURE 6B but with a references threshold
figure6B_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                 inner_join(openalex_threshold_references %>%
                                                              mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                              select(OA_source_ID, mains), by = "OA_source_ID") %>%
                                                 distinct(country, OA_source_ID, mains) %>%
                                                 count(country, mains, name = "n_journals") %>%
...
# merge only mainstream cases using full country names
figure6B_threshold <- world %>% left_join(figure6B_threshold %>% filter(mains == 1), by = c("admin" = "country"))

# group countries at the region level
figure6B_threshold <- figure6B_threshold %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                                              region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                                              region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                                              region_un == "Americas" & subregion == "South America" ~ "South America",
                                                                              TRUE ~ NA_character_))
...
# convert to long format
figure6B_threshold <- figure6B_threshold %>% select(name_long, region_custom, percent_journals, mains_label) %>%
                                             rename(percent = percent_journals)

figure6B_threshold <- figure6B_threshold %>% mutate(region_custom = case_when(name_long == "Mexico" ~ "North America", TRUE ~ region_custom))
figure6B_threshold <- figure6B_threshold %>% mutate(region_custom = recode(region_custom, "Central America & the Caribbean" = "Central America\n& the Caribbean"),
                                                    region_custom = factor(region_custom, levels = c("Africa", "South America", "Asia", "Europe", "North America", "Central America\n& the Caribbean", "Oceania")))

figure6B_threshold <- figure6B_threshold %>% filter(!is.na(percent), !is.na(region_custom)) %>%
                                             group_by(region_custom) %>%
                                             summarise(median_percent = median(percent, na.rm = TRUE)) %>%
                                             mutate(condition = "Threshold references")

figure6C <- bind_rows(figure6C, figure6B_threshold)
figure6C <- figure6C %>% st_drop_geometry() %>%
                         select(region_custom, condition, median_percent) %>%
                         pivot_wider(names_from = condition, values_from = median_percent) %>%
                         mutate(pp_change = `Threshold references` - `All references`)

# plot the difference in values between the medians of regions that consider all references vs the medians of regions that apply a threshold to references
ggplot(figure6C, aes(x = region_custom, y = pp_change, fill = region_custom)) +
  geom_col(width = 0.6) +
  geom_hline(yintercept = 0, linewidth = 0.8, color = "black") +
  scale_fill_manual(values = c("South America" = "#FEE08B", "Central America\n& the Caribbean" = "#FDAE61", "North America" = "#F46D43",
                               "Europe" = "#D53E4F", "Africa" = "#3288BD", "Asia" = "#66C2A5", "Oceania"  = "#E6F598")) +
  labs(x = "Region", y = "% point change") +
  theme_minimal() +
  theme(axis.text.x  = element_text(size = 7), axis.text.y  = element_text(size = 7),
        axis.title   = element_text(size = 8), legend.position = "none", panel.grid.minor = element_blank())
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig6C.png", width = 6, height = 2, dpi = 600)
...
ggplot(figure7A) +
  ...
  theme(legend.position = "bottom",
        ...
        panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig7A.png", width = 12, height = 5, dpi = 600)
...
ggplot(figure7B) +
  ...
  theme(legend.position = "bottom",
        ...
        panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig7B.png", width = 12, height = 5, dpi = 600)


## FIGURE 7C
# identify the local journals where each country has published during 2023, and compute the percentages of articles in mainstream and non-mainstream local journals, with reference threshold
figure7B_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                 inner_join(openalex_threshold_references %>%
                                                              mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                              select(OA_source_ID, mains), by = "OA_source_ID") %>%
                                                 group_by(country, mains) %>%
...
# merge only mainstream cases using full country names
figure7B_threshold <- world %>% left_join(figure7B_threshold %>% filter(mains == 1), by = c("admin" = "country"))

figure7C <- bind_rows(figure7B %>% st_drop_geometry() %>%
                        select(admin, percent_articles) %>%
                        mutate(condition = "All references"),
                      figure7B_threshold %>% st_drop_geometry() %>%
                        select(admin, percent_articles) %>%
                        mutate(condition = "Threshold references")) %>%
            pivot_wider(names_from = condition, values_from = percent_articles) %>%
            mutate(pp_change = `Threshold references` - `All references`)
...
figure7C %>%
  ...
  mutate(admin = recode(admin, "United Republic of Tanzania" = "United Republic\nof Tanzania"),
         admin = fct_reorder(admin, pp_change),
         direction = ifelse(pp_change >= 0, "positive", "negative")) %>%
  ggplot(aes(x = admin, y = pp_change, fill = direction)) +
  geom_col(width = 0.6) +
  geom_hline(yintercept = 0, linewidth = 0.8, color = "black") +
  scale_fill_manual(values = c("negative" = "#E6F598", "positive" = "#3288BD")) +
  labs(x = "Country",
       y = "% point change") +
  theme_minimal() +
  theme(axis.text.x  = element_text(size = 7, angle = 45, hjust = 1), axis.text.y  = element_text(size = 7),
        axis.title  = element_text(size = 8), legend.position = "none", panel.grid.minor = element_blank())
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig7C.png", width = 6, height = 2, dpi = 300)
...
figure8 %>% filter(!is.na(percent), !is.na(region_custom)) %>%
  ...
  theme(axis.text.x = element_text(size = 13),
        ...
        legend.position = "none",
        strip.text = element_text(size = 17, face = "bold"))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig8.png", width = 9, height = 4, dpi = 600)


## FIGURE 8C
# identify the local journals where each country has published during 2023, and compute the percentages of articles in non-English local journals, with reference threshold
figure8A_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                 inner_join(openalex_threshold_references %>%
                                                              mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                              select(OA_source_ID, langs), by = "OA_source_ID") %>%
                                                 group_by(country, langs) %>%
...
# merge only non-English cases using full country names
figure8A_threshold <- world %>% left_join(figure8A_threshold %>% filter(langs == 0), by = c("admin" = "country"))

# group countries at the region level
figure8A_threshold <- figure8A_threshold %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                                              region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                                              region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                                              region_un == "Americas" & subregion == "South America" ~ "South America",
                                                                              TRUE ~ NA_character_))
...
# convert to long format
figure8A_threshold <- figure8A_threshold %>% select(name_long, region_custom, percent_articles, langs_label) %>%
                                             rename(percent = percent_articles) %>%
                                             mutate(metric = "Non-English")

# identify the local journals where each country has published during 2023, and compute the percentages of articles in open-access local journals, with reference threshold
figure8B_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                 inner_join(openalex_threshold_references %>%
                                                 mutate(OA_source_ID = as.character(OA_source_ID),
                                                        is_OA = map_lgl(open_access, ~ {if (is.null(.x)) return(FALSE)
                                                          .x$SCOP_open_access == "Unpaywall Open Access" |
...
# merge only open-access cases using full country names
figure8B_threshold <- world %>% left_join(figure8B_threshold %>% filter(is_OA == TRUE), by = c("admin" = "country"))

# group countries at the region level
figure8B_threshold <- figure8B_threshold %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                                              region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                                              region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                                              region_un == "Americas" & subregion == "South America" ~ "South America",
                                                                              TRUE ~ NA_character_))
...
# convert to long format
figure8B_threshold <- figure8B_threshold %>% select(name_long, region_custom, percent_articles, OA_label) %>%
                                             rename(percent = percent_articles) %>%
                                             mutate(metric = "Open Access")

# combine both figure8 A & B into a single dataframe for plotting purposes
figure8_threshold <- bind_rows(figure8A_threshold %>% rename(label = langs_label),
                               figure8B_threshold %>% rename(label = OA_label))

figure8_threshold <- figure8_threshold %>% mutate(region_custom = case_when(name_long == "Mexico" ~ "North America", TRUE ~ region_custom))
figure8_threshold <- figure8_threshold %>% mutate(region_custom = recode(region_custom, "Central America & the Caribbean" = "Central America\n& the Caribbean"),
                                                  region_custom = factor(region_custom, levels = c("North America", "Asia", "Africa", "Europe", "Oceania", "Central America\n& the Caribbean", "South America")))

figure8C <- bind_rows(figure8 %>% st_drop_geometry() %>% select(name_long, region_custom, percent, metric) %>%
                        mutate(condition = "All references"),
                      figure8_threshold %>% st_drop_geometry() %>%
                        select(name_long, region_custom, percent, metric) %>%
                        mutate(condition = "Threshold references")) %>%
            filter(!is.na(percent), !is.na(region_custom)) %>%
            group_by(region_custom, metric, condition) %>%
            summarise(median_percent = median(percent, na.rm = TRUE), .groups = "drop") %>%
            pivot_wider(names_from = condition, values_from = median_percent) %>%
            mutate(pp_change = `Threshold references` - `All references`)

ggplot(figure8C, aes(x = region_custom, y = pp_change, fill = region_custom)) +
  geom_col(width = 0.6) +
  geom_hline(yintercept = 0, linewidth = 0.8, color = "black") +
  ...
  theme(axis.text.x  = element_text(size = 7), axis.text.y  = element_text(size = 7),
        axis.title   = element_text(size = 8), legend.position = "none", panel.grid.minor = element_blank())
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig8C.png", width = 6, height = 2.5, dpi = 600)
...
ggplot(figure9A, aes(x = perc, y = OA_main_domains, fill = mains_label)) +
  ...
  theme(strip.text = element_text(size = 12, face = "bold"), axis.text.y = element_text(size = 7),
        legend.position = "bottom", legend.direction = "horizontal", legend.key.size = unit(0.4, "cm"), legend.text = element_text(size = 7))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig9A.png", width = 6, height = 2, dpi = 300)


## FIGURE 9B
figure9B <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
...
# merge only mainstream cases using full country names
figure9B <- world %>% left_join(figure9B %>% filter(mains == 1), by = c("admin" = "country"))

# group countries at the region level
figure9B <- figure9B %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                          region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                          region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                          region_un == "Americas" & subregion == "South America" ~ "South America",
                                                          TRUE ~ NA_character_))
...
# convert to long format
figure9B <- figure9B %>% select(name_long, region_custom, percent_journals, mains_label) %>%
                         rename(percent = percent_journals)
...
figure9B %>% filter(!is.na(percent), !is.na(region_custom)) %>%
  ...
  theme(axis.text.x = element_text(size = 13), axis.text.y = element_text(size = 13),
        axis.title = element_text(size = 14), legend.position = "none")
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig9B.png", width = 9, height = 4, dpi = 600)


## FIGURE 9C
figure9C <- figure9B %>% filter(!is.na(percent), !is.na(region_custom)) %>%
                         group_by(region_custom) %>%
                         summarise(median_percent = median(percent, na.rm = TRUE)) %>%
                         mutate(condition = "All citations")

# apply a 7 citations threshold to total citations per journal which is the maximum value before loosing whole regions
openalex_threshold_citations <- local_variable_cits %>% filter(cits_total >= 7)

# produce the same computations as in FIGURE 9B but with a citations threshold
figure9B_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                 filter(OA_source_ID %in% as.character(openalex_threshold_citations$journal_id)) %>%
                                                 inner_join(local_journals_recipients %>%
                                                              mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                              select(OA_source_ID, mains), by = "OA_source_ID") %>%
                                                 distinct(country, OA_source_ID, mains) %>%
...
# merge only mainstream cases using full country names
figure9B_threshold <- world %>% left_join(figure9B_threshold %>% filter(mains == 1), by = c("admin" = "country"))

# group countries at the region level
figure9B_threshold <- figure9B_threshold %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                                              region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                                              region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                                              region_un == "Americas" & subregion == "South America" ~ "South America",
                                                                              TRUE ~ NA_character_))
...
# convert to long format
figure9B_threshold <- figure9B_threshold %>% select(name_long, region_custom, percent_journals, mains_label) %>%
                                             rename(percent = percent_journals)

figure9B_threshold <- figure9B_threshold %>% mutate(region_custom = case_when(name_long == "Mexico" ~ "North America", TRUE ~ region_custom))
figure9B_threshold <- figure9B_threshold %>% mutate(region_custom = recode(region_custom, "Central America & the Caribbean" = "Central America\n& the Caribbean"),
                                             region_custom = factor(region_custom, levels = c("Africa", "Asia", "Central America\n& the Caribbean", "South America", "North America", "Europe", "Oceania")))

figure9B_threshold <- figure9B_threshold %>% filter(!is.na(percent), !is.na(region_custom)) %>%
                                             group_by(region_custom) %>%
                                             summarise(median_percent = median(percent, na.rm = TRUE)) %>%
                                             mutate(condition = "Threshold citations")

figure9C <- bind_rows(figure9C, figure9B_threshold)

figure9C <- figure9C %>% st_drop_geometry() %>%
                         select(region_custom, condition, median_percent) %>%
                         pivot_wider(names_from = condition, values_from = median_percent) %>%
                         mutate(pp_change = `Threshold citations` - `All citations`)

# plot the difference in values between the medians of regions that consider all citations vs the medians of regions that apply a threshold to citations
ggplot(figure9C, aes(x = region_custom, y = pp_change, fill = region_custom)) +
  geom_col(width = 0.6) +
  geom_hline(yintercept = 0, linewidth = 0.8, color = "black") +
  scale_fill_manual(values = c("South America" = "#FEE08B", "Central America\n& the Caribbean" = "#FDAE61", "North America" = "#F46D43",
                               "Europe" = "#D53E4F", "Africa" = "#3288BD", "Asia" = "#66C2A5", "Oceania"  = "#E6F598")) +
  ...
  theme(axis.text.x  = element_text(size = 7), axis.text.y  = element_text(size = 7),
        axis.title   = element_text(size = 8), legend.position = "none", panel.grid.minor = element_blank())
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig9C.png", width = 6, height = 2, dpi = 600)
...
ggplot(figure10A) +
  ...
  theme(legend.position = "bottom",
        ...
        panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig10A.png", width = 12, height = 5, dpi = 600)
...
ggplot(figure10B) +
  ...
  theme(legend.position = "bottom",
        ...
        panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig10B.png", width = 12, height = 5, dpi = 600)


## FIGURE 10C
# identify the local journals where each country has published during 2023, and compute the percentages of articles in mainstream and non-mainstream local journals, with citation threshold
figure10B_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                  inner_join(openalex_threshold_citations %>%
                                                               mutate(journal_id = as.character(journal_id)) %>%
                                                               left_join(ddff_megamerge %>%
                                                                           mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                                           select(OA_source_ID, mains), by = c("journal_id" = "OA_source_ID")) %>%
                                                               select(journal_id, mains), by = c("OA_source_ID" = "journal_id")) %>%
                                                  group_by(country, mains) %>%
                                                  summarise(n_articles = sum(article_count, na.rm = TRUE),
                                                            .groups = "drop") %>%
                                                  group_by(country) %>%
...
# merge only mainstream cases using full country names
figure10B_threshold <- world %>% left_join(figure10B_threshold %>% filter(mains == 1), by = c("admin" = "country"))

figure10C <- bind_rows(figure10B %>% st_drop_geometry() %>%
                         select(admin, percent_articles) %>%
                         mutate(condition = "All citations"), figure10B_threshold %>% st_drop_geometry() %>%
                         select(admin, percent_articles) %>%
                         mutate(condition = "Threshold citations")) %>%
  pivot_wider(names_from = condition, values_from = percent_articles) %>%
  mutate(pp_change = `Threshold citations` - `All citations`)
...
figure10C %>%
  ...
  arrange(pp_change) %>%
  {bind_rows(slice_head(., n = 10), slice_tail(., n = 10))} %>%
  mutate(admin = recode(admin, "Trinidad and Tobago" = "Trinidad and\nTobago",
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45, hjust = 1), axis.text.y = element_text(size = 7),
        axis.title  = element_text(size = 8), legend.position = "none", panel.grid.minor = element_blank())
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig10C.png", width = 6, height = 2, dpi = 300)
...
figure11 %>% filter(!is.na(percent), !is.na(region_custom)) %>%
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(size = 13), axis.text.y = element_text(size = 13),
        axis.title = element_text(size = 14), legend.position = "none", strip.text = element_text(size = 17, face = "bold"))
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig11.png", width = 9, height = 4, dpi = 600)


## FIGURE 11C
# identify the local journals where each country has published during 2023, and compute the percentages of articles in non-English local journals, with citation threshold
figure11A_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                  inner_join(openalex_threshold_citations %>%
                                                               mutate(journal_id = as.character(journal_id)) %>%
                                                               left_join(ddff_megamerge %>%
                                                                           mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                                           select(OA_source_ID, langs), by = c("journal_id" = "OA_source_ID")) %>%
                                                               select(journal_id, langs), by = c("OA_source_ID" = "journal_id")) %>%
                                                  group_by(country, langs) %>%
                                                  summarise(n_articles = sum(article_count, na.rm = TRUE), .groups = "drop") %>%
                                                  group_by(country) %>%
                                                  mutate(total_articles = sum(n_articles),
                                                         percent_articles = 100 * n_articles / total_articles,
                                                         langs_label = ifelse(langs == 1, "English local journals",
...
# merge only non-English cases using full country names
figure11A_threshold <- world %>% left_join(figure11A_threshold %>% filter(langs == 0), by = c("admin" = "country"))

# group countries at the region level
figure11A_threshold <- figure11A_threshold %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                                                region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                                                region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                                                region_un == "Americas" & subregion == "South America" ~ "South America",
                                                                                TRUE ~ NA_character_))
...
# convert to long format
figure11A_threshold <- figure11A_threshold %>% select(name_long, region_custom, percent_articles, langs_label) %>%
                                               rename(percent = percent_articles) %>%
                                               mutate(metric = "Non-English")

# identify the local journals where each country has published during 2023, and compute the percentages of articles in open-access local journals, with citation threshold
figure11B_threshold <- openalex_articles_2023 %>% mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                  inner_join(openalex_threshold_citations %>%
                                                               mutate(journal_id = as.character(journal_id)) %>%
                                                               left_join(ddff_megamerge %>%
                                                                           mutate(OA_source_ID = as.character(OA_source_ID)) %>%
                                                                           select(OA_source_ID, open_access), by = c("journal_id" = "OA_source_ID")) %>%
                                                               mutate(is_OA = map_lgl(open_access, ~ {if (is.null(.x)) return(FALSE)
                                                                 .x$SCOP_open_access == "Unpaywall Open Access" |
                                                                   .x$DOAJ_open_access == "Yes" |
                                                                   .x$OA_open_access == TRUE})) %>%
                                                               select(journal_id, is_OA), by = c("OA_source_ID" = "journal_id")) %>%
                                                  group_by(country, is_OA) %>%
                                                  summarise(n_articles = sum(article_count, na.rm = TRUE), .groups = "drop") %>%
                                                  group_by(country) %>%
                                                  mutate(total_articles = sum(n_articles),
...
# merge only open-access cases using full country names
figure11B_threshold <- world %>% left_join(figure11B_threshold %>% filter(is_OA == TRUE), by = c("admin" = "country"))

# group countries at the region level
figure11B_threshold <- figure11B_threshold %>% mutate(region_custom = case_when(region_un %in% c("Africa", "Asia", "Europe", "Oceania") ~ region_un,
                                                                                region_un == "Americas" & subregion == "Northern America" ~ "North America",
                                                                                region_un == "Americas" & subregion %in% c("Central America", "Caribbean") ~ "Central America & the Caribbean",
                                                                                region_un == "Americas" & subregion == "South America" ~ "South America",
                                                                                TRUE ~ NA_character_))
...
# convert to long format
figure11B_threshold <- figure11B_threshold %>% select(name_long, region_custom, percent_articles, OA_label) %>%
                                               rename(percent = percent_articles) %>%
                                               mutate(metric = "Open Access")

# combine both figure11 A & B into a single dataframe for plotting purposes
figure11_threshold <- bind_rows(figure11A_threshold %>% rename(label = langs_label),
                                figure11B_threshold %>% rename(label = OA_label))

figure11_threshold <- figure11_threshold %>% mutate(region_custom = case_when(name_long == "Mexico" ~ "North America", TRUE ~ region_custom))
figure11_threshold <- figure11_threshold %>% mutate(region_custom = recode(region_custom, "Central America & the Caribbean" = "Central America\n& the Caribbean"),
                                                    region_custom = factor(region_custom, levels = c("North America", "Asia", "Africa", "Europe", "Oceania", "Central America\n& the Caribbean", "South America")))

figure11C <- bind_rows(figure11 %>% st_drop_geometry() %>% select(name_long, region_custom, percent, metric) %>%
                         mutate(condition = "All citations"), figure11_threshold %>% st_drop_geometry() %>%
                         select(name_long, region_custom, percent, metric) %>%
                         mutate(condition = "Threshold citations")) %>%
                       filter(!is.na(percent), !is.na(region_custom)) %>%
                       group_by(region_custom, metric, condition) %>%
...
ggplot(figure11C, aes(x = region_custom, y = pp_change, fill = region_custom)) +
  geom_col(width = 0.6) +
  geom_hline(yintercept = 0, linewidth = 0.8, color = "black") +
  facet_wrap(~ metric, ncol = 1) +
  scale_fill_manual(values = c("South America" = "#FEE08B", "Central America\n& the Caribbean" = "#FDAE61", "North America" = "#F46D43",
  ...
  theme(axis.text.x  = element_text(size = 7), axis.text.y  = element_text(size = 7),
        axis.title   = element_text(size = 8), legend.position = "none", panel.grid.minor = element_blank())
ggsave("~/Desktop/OpenAlex_journals_dataset/figures/Fig11C.png", width = 6, height = 2.5, dpi = 600)
